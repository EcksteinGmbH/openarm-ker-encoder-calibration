...
int main(void){
  ...
  for(uint8_t set=0; set<20; set++){
    ...
    for(uint8_t k=0;k<6;k++){
      ...
      else if(set<14){ c.amp[k]=(int16_t)rnd(); c.phase[k]=rnd(); }                    /* random */
    }
    if(set==13) c.n=3; if(set>=14){ for(uint8_t k=0;k<6;k++){c.amp[k]=32767;c.phase[k]=(uint16_t)(k*11111u);} c.n=set-14; for(uint8_t k=c.n;k<6;k++){c.amp[k]=0;c.phase[k]=0;} }                                                        /* short N */
