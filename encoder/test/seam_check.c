 ...
 * MODIFICATION NOTICE: new file, not present in upstream OpenArm. Copied from
 * docs/evidence/v3-reference/seam_check.c with the include retargeted at the firmware
 * source; run_tests.sh checks the two copies have not drifted apart. */
/* Seam coverage of the differential-test sets (derived from diff_main.c).
 * Original header: Differential test: identical source built for host and for AVR. Each prints one FNV-1a
...
int main(void){
  ...
  for(uint8_t set=0; set<21; set++){
    ...
    for(uint16_t r=0;r<32768;r++){
      int32_t base = c.n ? (int32_t)((uint32_t)r<<6) : (int32_t)(((uint32_t)r<<6)|(r>>9));
      int32_t d = (int32_t)ker_compensate(r,&c) - base;          /* wrapped correction */
      if(d >  (1L<<20)) d -= (1L<<21);
      if(d < -(1L<<20)) d += (1L<<21);
      if(base + d < 0) below++; else if(base + d >= (1L<<21)) above++;
    }
    printf("set %2u (N=%u): outputs wrapped below 0: %5lu, above 2^21-1: %5lu\n",set,c.n,(unsigned long)below,(unsigned long)above);
